...
-These functions were defined in Python3. NumPy and scipy.stats functions norm, t, chisquare and chi2 are necessary to run these. This was because these functions are designed to assist me specifically when running these modules anyway, as their purpose is to offer statistical inference on datasets.
...
5) z_conf_intv(phat, n, confidence) 
...
6) diff_z_intv(phat1, n1, phat2, n2, confidence)
This function calculates a confidence interval (confidence defaults to 95% but can be changed) for the difference of two proportions. The basic structure and calculations are very similar to the previous function, tweaked only to allow for differences to be subtracted between two means, and their variances added together. Returns a similar (min, max) range.

7) t_conf_intv(bar, stdev, n, confidence)
...
8) diff_t_intv(sample1, sample2, confidence)
...
9) one_samp_z_test(phat, p0, n, both_tails = False, alpha = None)
...
10) two_samp_z_test(sample1, sample2, both_tails = False)

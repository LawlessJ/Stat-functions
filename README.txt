...
a flat list [pop1 expected data, pop2 expected data, pop3 expected data]. With this, you can pass into expected in chisquare() to properly calculate a homogeneity test. If doing so, be sure to define the degrees of freedom as (# of populations -1) * (# of items in populations -1)

format would be chi_2, pval =scipy.stats.chisquare([actual data in one flat list], expected, ddof = (# of populations -1) * (# of parameters measured -1)
resulting chi_2 would be correct, but pval should be changed --> pval = 1 - chi2.cdf(chi_2, ddof)
